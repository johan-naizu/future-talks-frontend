...
const Footer = () => {
    ...
    () => [
      {
        name: "instagram",
        icon: faInstagram,
        href: "https://www.instagram.com/_future.talks",
      },
      {
        name: "facebook",
        icon: faFacebook,
        href: "https://www.facebook.com/share/6288ERLXGzH12skb/",
      },
    ],
    [],
  );
  ...
  const contacts = useMemo(
    () => [
      {
        name: "dmfuturetalks@gmail.com",
        href: "mailto:dmfuturetalks@gmail.com",
      },
      {
        name: "+91 8891681655",
        href: "tel:+918891681655",
      },
    ],
    [],
  );
  ...
  return (
    <footer className="grid grid-rows-8 grid-cols-4 bg-primarycolor h-96 w-screen text-white text-[0.75rem] overflow-hidden">
      <div
        className={`col-start-1 col-span-4 row-start-1 row-span-3 p-2 text-xl sm:text-3xl xl:text-6xl flex items-center justify-center text-center border border-0 border-b-2 ${subjectivity.className}`}
      >
        <span className="hidden lg:inline w-full 2xl:text-[5rem]">
          <Marquee speed={150}>JOIN US ON THE ROAD TO SUCCESS</Marquee>
        </span>
        <span className="lg:hidden">JOIN US ON THE ROAD TO SUCCESS</span>
      </div>
      <div className="col-start-1 col-span-2 row-start-4 row-span-4 p-2 border border-0 border-b-2 border-e-2 relative">
        <div className="overflow-hidden w-full lg:w-1/2 p-2">
          Do you want to know which college to join or which course to pursue?{" "}
          <br /> Get in touch with us!
        </div>

        <button
          className={`bg-white text-${
            hover ? "white" : "primarycolor"
          } ps-4 absolute bottom-4 left-4 font-bold flex items-center justify-center gap-4 z-0 ${
